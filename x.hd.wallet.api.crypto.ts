...
export class XHDWalletAPI {
  ...
  private async rawSign(
    ...
    const r = lws.crypto_core_ed25519_scalar_reduce(
      lws.crypto_hash_sha512(Buffer.concat([kR, data]))
    );
    ...
    let h = lws.crypto_core_ed25519_scalar_reduce(
      lws.crypto_hash_sha512(Buffer.concat([R, publicKey, data]))
    );

    // \(5): S = (r + h * k) mod q
    const S = lws.crypto_core_ed25519_scalar_add(
      r,
      lws.crypto_core_ed25519_scalar_mul(h, scalar)
    );
  ...
  async ECDH(
    ...
    const ourPub: Uint8Array =
      lws.crypto_scalarmult_ed25519_base_noclamp(scalar);

    // convert from ed25519 to curve25519
    const ourPubCurve25519: Uint8Array =
      lws.crypto_sign_ed25519_pk_to_curve25519(ourPub);
    const otherPartyPubCurve25519: Uint8Array =
      lws.crypto_sign_ed25519_pk_to_curve25519(otherPartyPub);

    // find common point
    const sharedPoint: Uint8Array = lws.crypto_scalarmult(
      scalar,
